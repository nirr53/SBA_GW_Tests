...
/**
* ----------------
* This test tests sending of differnt CPU status alarms.
* -----------------
* 
* Tests:
*    1. Check that page is opened clear and correct
*    2. Test CPU status alarm with warning severity	
*    3. Test CPU status alarm with minor severity
*    4. Test CPU status alarm with major severity
*    5. Test CPU status alarm with critical severity
*    6. Test CPU status alarm with cleared severity
*    7. Test CPU status alarm with indeterminate severity
*    
*    Results:
...
public class Test217_Tools_Alarms_Testser_sba_cpu_status_alarm {
  ...
  public Test217_Tools_Alarms_Testser_sba_cpu_status_alarm(String browser) {
  ...
  @Test
  public void Tools_Alarms_Testser_sba_cpu_status_alarm() throws Exception {
	...
	String alarmType   = "SBA CPU Status Alarm";
	...
	// Step 2 - test cpu status alarm with warning severity
	testFuncs.myDebugPrinting("Step 2 - test cpu status alarm with warning severity", testVars.logerVars.MAJOR);
	sendAlarmViaTester(driver, alarmType, "Warning", ipSource, description);
	
	// Step 3 - test cpu status alarm with minor severity
	testFuncs.myDebugPrinting("Step 3 - test cpu status alarm with minor severity", testVars.logerVars.MAJOR);
	sendAlarmViaTester(driver, alarmType, "Minor", ipSource, description);
            	
	// Step 4 - test cpu status alarm with major severity
	testFuncs.myDebugPrinting("Step 4 - test cpu status alarm with major severity", testVars.logerVars.MAJOR);
	sendAlarmViaTester(driver, alarmType, "Major", ipSource, description);
	
	// Step 5 - test cpu status alarm with critical severity
	testFuncs.myDebugPrinting("Step 5 - test cpu status alarm with critical severity", testVars.logerVars.MAJOR);
	sendAlarmViaTester(driver, alarmType, "Critical", ipSource, description);

	// Step 6 - test cpu status alarm with cleared severity
	testFuncs.myDebugPrinting("Step 6 - test cpu status alarm with cleared severity", testVars.logerVars.MAJOR);
	sendAlarmViaTester(driver, alarmType, "Cleared", ipSource, description);
	
	// Step 7 - test cpu status alarm with indeterminate severity
	testFuncs.myDebugPrinting("Step 7 - test cpu status alarm with indeterminate severity", testVars.logerVars.MAJOR);
